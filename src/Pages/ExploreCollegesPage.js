...
const ExploreCollegesPage = () => {
    ...
    const fetchUniversities = async () => {
      try {
        const res = await axios.get(
          "https://acvora-07fo.onrender.com/api/universities"
        );
        if (res.data.success) {
  ...
  const getFilteredOptions = () => {
    ...
    switch (activeTab) {
      ...
      case "specialization":
        return specializations.filter((opt) =>
      ...
      case "institute":
        return instituteTypes.filter((opt) =>
          opt.toLowerCase().includes(lowerSearch)
        );
      case "management":
        return managementTypes.filter((opt) =>
          opt.toLowerCase().includes(lowerSearch)
        );
      default:
        return [];
  ...
  // ✅ Filtering logic updated to only show 'approved' colleges
  const filteredColleges = universities.filter((uni) => {
    // 1. New Condition: Only include universities with an 'approved' status
    const isApproved = uni.status === "approved"; //

    // 2. Existing search and filter conditions
    ...
    const matchesDegree =
      ...
      degreeFilter.some((deg) =>
        uni.courses?.some((c) =>
          c.courseName?.toLowerCase().includes(deg.toLowerCase())
        )
      );
    ...
    const matchesExam =
      ...
      examFilter.some((exam) =>
        uni.admissions?.some((a) =>
          a.eligibility?.toLowerCase().includes(exam.toLowerCase())
        )
      );
    ...
    const matchesInstituteType =
      instituteTypeFilter.length === 0 ||
      instituteTypeFilter.includes(uni.type);

    const matchesManagementType =
      managementTypeFilter.length === 0 ||
      managementTypeFilter.includes(uni.ownership);

    // Return true only if the college matches all filters AND is approved
    return (
      isApproved && // ✅ This is the key change
      matchesSearch &&
      matchesDegree &&
      matchesExam &&
                    ...
                    filteredOptions.map((mgmt) => (
                      <label key={mgmt}>
                        <input
                          ...
                          onChange={() =>
                            handleFilterChange("management", mgmt)
                          }
                        />
                        {mgmt}
                      </label>
